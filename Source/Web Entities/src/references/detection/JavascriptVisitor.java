...
import references.JsFunctionCall;
import references.JsFunctionDecl;
import references.JsRefToHtmlForm;
import references.JsRefToHtmlId;
import references.JsRefToHtmlInput;
import references.JsVariable;
...
public class JavascriptVisitor extends ASTVisitor {
	...
	private Location javascriptLocation;
	 ...
	 * Constructor
	 */
	public JavascriptVisitor(Location javascriptLocation, Constraint constraint, ReferenceManager referenceManager) {
	...
	private Location getLocation(ASTNode astNode) {
		return getLocation(astNode, 0);
	}
	
	private Location getLocation(ASTNode astNode, int adjustedOffset) {
		return new SingleLocation(javascriptLocation, astNode.getStartPosition() + adjustedOffset);
	}
